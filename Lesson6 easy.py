# Задаzа-1: Написать класс для фигуры-треугольника, заданного координатами трех точек.
...
class trapeze:
    ...
    def Ptrp(self):
        return print("Периметр равен",round((self.baseA + self.baseB + self.baseD + self.baseC),2))

    def Strp(self):
        print("Площадь равна ", round(((self.baseD + self.baseB)/2 * (sqrt((abs(self.baseA **2- (((self.baseD -self.baseB)**2)/4)))))),2))
